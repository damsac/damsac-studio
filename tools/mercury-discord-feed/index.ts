...
function loadBotToken(): string {
  ...
  try {
    ...
    if (!match) throw new Error("DISCORD_BOT_TOKEN not found in " + envPath);
    return match[1].trim();
  } catch (e: any) {
    if (e.code === "ENOENT") {
      throw new Error(
        `Discord bot token not found. Set DISCORD_BOT_TOKEN env var or create ${envPath}`
      );
    }
    throw e;
  }
...
function loadConfig(): Config {
  return {
    ...
    feedChannelId: process.env.DISCORD_FEED_CHANNEL_ID || "1485368650201301062",
    mercuryDbPath:
...
async function main() {
  ...
  client.once("clientReady", async () => {
    ...
    // Startup message
    await feedChannel.send("🟢 Mercury feed online — watching all channels");
    ...
    if (backlogMessages.length > 100) {
      ...
      await feedChannel.send(
        `📋 Catching up: skipped ${backlogMessages.length} historical messages since ${oldestTs}. Starting from current.`
      );
      cursor = skipTo;
  ...
  async function shutdown(signal: string) {
    ...
    if (feedChannel) {
      try {
        await feedChannel.send("🔴 Mercury feed offline");
      } catch {
        // Best effort
      }
    }
